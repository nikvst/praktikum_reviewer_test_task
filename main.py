...
# здесь нужен один дополнительный отступ
# https://www.python.org/dev/peps/pep-0008/#blank-lines
class Record:
    # Здесь у аргмента date значением по умолчанию указана пустая строка, но 
    # лучше будет указать None.
...
class Calculator:
    ...
    def get_today_stats(self):
        ...
        #    или можно отбросить квадратные скобки и сделать из этого генератор
        for Record in self.records:
            # получение сегодняшней даты лучше вынести за пределы цикла, чтобы
            # не делать одну и ту же работу на каждой иттерации
    ...
    def get_week_stats(self):
        ...
        for record in self.records:
            ...
            # пробелов (а после "-" их слишком много).
...
class CashCalculator(Calculator):
    ...
    # 3. аргументы не должны называться в заглавных сивмолах
    ...
    def get_today_cash_remained(self, currency, USD_RATE=USD_RATE, EURO_RATE=EURO_RATE):
        ...
        # нет оступов вокруг ==
            ...
            # Здесь происходит сравнение, но оно ни на что не влияет.
            # Эту строку можно в принципе удалить
            cash_remained == 1.00
            # нет оступа после =
            currency_type ='руб'
        # тут явно следует поставить отступ, чтобы два блока с if
        # не сливались в один
        if cash_remained > 0:
